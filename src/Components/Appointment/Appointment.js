...
const Appointment = () => {
    ...
    return (
        <div>
            ...
            <AvailableAppointment date={date} bookingSuccess={bookingSuccess}></AvailableAppointment>
            <Container sx={{ mt: 3 }}>
                <Box sx={{ flexGrow: 1 }}>
                    <Grid container spacing={1}>
                        {docList.map((doc) => (
                            <DocCard setBookingSuccess={setBookingSuccess} key={doc.id} doc={doc} date={date}></DocCard>
                        ))}
                    </Grid>
                </Box>
            </Container>
